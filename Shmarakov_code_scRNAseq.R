...
# Read in all four input expression matrices from disc C
...
# Read in all input matrices from disc D
celseq.data <- Read10X(data.dir = ("D:/WI001/WI001_cellranger_count_outs/filtered_feature_bc_matrix"))
celseq2.data <- Read10X(data.dir = ("D:/WI02/WI02_Analysis/WI02_Analysis_Extracted/WI02_cellranger_count_outs/filtered_feature_bc_matrix"))
...
# using use Seurat's FeatureScatter function to create scatterplots of the relationships among QC variables
...
##use a bootstrapping technique called Jackstraw analysis to estimate a p-value for each component, print out a plot, and save the p-values to a file
...
# Remember, the clustering is stored in @meta.data in column seurat_clusters and the technology is stored in the column tech. Remember you can also use DimPlot
...
# Visualize the the batches on the UMAP
...
# Identify anchors on the 2 datasets, commonly shared variable genes across samples, and integrate samples.
...
# Clustering. Choose the dimensional reduction type to use and the number of aligned canonical correlation vectors to use.
gcdata <- FindNeighbors(gcdata, reduction = "pca", dims = 1:15, k.param = 20)
...
# UMAP. Choose the dimensional reduction type to use and the number of aligned canonical correlation vectors to use.
gcdata <- RunUMAP(gcdata, dims = 1:15, reduction = "pca", n.neighbors = 15, min.dist = 0.5, spread = 1, metric = "euclidean", seed.use = 1)  
...
# We can also identify conserved marker genes across the batches. Differential gene expression is done across each batch, and the p-values are combined.
...
# choose the top 10 DEGs in each cluster, and print them to a heatmap using DoHeatmap and a red/white/blue color scheme
...
features<- c("Col1a1", "Pdgfra", "Ptgis", "Epcam", "Sftpa1", "Sftpb", "Pecam1", "Cdh5", "Plvap", "Ptprc", "Il1b", "Lsp1")
...
features<- c("Epcam", "Sftpa1", "Sftpb", "Sftpc", "Abca3", "Napsa", "Aqp5","Ager", "Pdpn", "Hopx", "Cav1", "Scnn1g", "Scnn1b")
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Intergated_AEC_markers.jpg"), width = 10, height = 16, units="in", res=300);
p101 <- StackedVlnPlot(obj = gcdata, features = features)
print(plot_grid(p101));
dev.off();

features<- c("Epcam", "Cdh1", "Elf3", "Grhl2", "Sftpa1", "Abca3", "Hopx", "Pdpn", "Ager")
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Intergated_AT_markers.jpg"), width = 10, height = 12, units="in", res=300);
p26 <- StackedVlnPlot(obj = gcdata, features = features)
print(plot_grid(p26));
dev.off();
...
## Reclustering fibroblast clusters

DefaultAssay(CFonly) <- "integrated"

## Can omit at this point
CFonly <- RunUMAP(CFonly, dims = 1:15, reduction = "pca", n.neighbors = 15, min.dist = 0.5, spread = 1, metric = "euclidean", seed.use = 1)  

# Clustering. Choose the dimensional reduction type to use and the number of aligned canonical correlation vectors to use.
CFonly <- FindNeighbors(CFonly, reduction = "pca", dims = 1:15, k.param = 20)

#Clustering with different resolutions
CFonly <- FindClusters(CFonly, resolution = 0.2, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.3, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.4, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.5, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.6, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.7, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.8, algorithm = 1, random.seed = 100)
CFonly <- FindClusters(CFonly, resolution = 0.9, algorithm = 1, random.seed = 100)

# Building clustering tree to know cluster resolution value
library(clustree)
head(CFonly[[]])
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Clustree_CFonly.jpg"), width = 10, height = 8, units="in", res=300);
p41 <- clustree(CFonly)
print(plot_grid(p41));
dev.off();
...
# Remember, the clustering is stored in @meta.data in column seurat_clusters 
# and the technology is stored in the column tech. Remember you can also use DimPlot.
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/UMAP_Clusters_CFonly_0_2.jpg"), width = 8, height = 8, units="in", res=300);
p55 <- DimPlot(CFonly, reduction = "umap", group.by = "seurat_clusters")
print(plot_grid(p55));
dev.off();

# performing DEG analysis on all clusters simultaneously using the default differential expression test (Wilcoxon), then save the results to a file.
DEGs <- FindAllMarkers(object=CFonly, logfc.threshold=1, min.diff.pct=.2);
write.table(DEGs, file=sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/DEGs.Wilcox_CFonly_0_2.xls"), quote=FALSE, sep="/", row.names=FALSE);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Hurskainen_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Liu_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Xie_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Travaglini_Cfonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Tsukui_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Fibroblast_Genes_Grouped_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Fibroblast_Cluster_Markers_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
...
jpeg(sprintf("C:/Users/igors/Documents/Igor_docs_2019/Lung/SingleCellTranscriptomics/Seurat_Analysis/Batch_corrected/Fibroblast_Cluster_Markers_Refined_CFonly.jpg"), width = 10, height = 4, units="in", res=300);
p18 <- DotPlot(obj = CFonly, features = features) + theme(axis.text.x = element_text(angle = 90, hjust=1))
print(plot_grid(p18));
dev.off();